/**
 * useStreamGenerateAnalysisPlan hook 单元测试（SPEC 0021 分析方案生成流式化）。
 *
 * 测试覆盖：
 ...
 * - reset()：状态重置为初始值
 *
 * 通过 mock api.streamGenerateAnalysisPlan 返回异步迭代器模拟 SSE 事件流。
 */
...
import { useStreamGenerateAnalysisPlan } from "../hooks";
...
describe("useStreamGenerateAnalysisPlan - 成功路径", () => {
  it("chunk 事件累积到 chunks（done 前可见）", async () => {
    const { Wrapper } = createWrapper();
    const streamSpy = vi
      .spyOn(api, "streamGenerateAnalysisPlan")
      .mockReturnValue(
        makeMockStream([chunkEvent("hello"), chunkEvent(" world")])
      );

    const { result } = renderHook(
      () => useStreamGenerateAnalysisPlan(PROJECT_ID, DATASET_ID),
      { wrapper: Wrapper }
    );
  ...
  it("done 事件设置 result 并清空 chunks", async () => {
    const { Wrapper } = createWrapper();
    vi.spyOn(api, "streamGenerateAnalysisPlan").mockReturnValue(
      makeMockStream([
        chunkEvent('{"cleaning_plan":['),
        chunkEvent('{"field":"age"}]}'),
  ...
  it("done 事件触发 invalidate analysis list query", async () => {
    ...
    // 应触发分析方案列表 query 的 invalidate
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: ["analysis", PROJECT_ID, "list"],
    });
  });
  ...
  it("start 时重置旧状态", async () => {
    const { Wrapper } = createWrapper();
    // 第一次流式：返回 error
    vi.spyOn(api, "streamGenerateAnalysisPlan")
  ...
  it("done 事件包含 fallback_used 标记", async () => {
    const { Wrapper } = createWrapper();
    vi.spyOn(api, "streamGenerateAnalysisPlan").mockReturnValue(
      makeMockStream([doneEvent("plan_001", "LOCAL_RULE", true)])
    );
    ...
    expect(result.current.result?.plan_id).toBe("plan_001");
  });
...
describe("useStreamGenerateAnalysisPlan - error 事件", () => {
  it("error 事件设置错误信息并保留 partial_text", async () => {
    ...
    vi.spyOn(api, "streamGenerateAnalysisPlan").mockReturnValue(
      makeMockStream([
        ...
        chunkEvent('"部分内容"}'),
        errorEvent(
          "ANALYSIS_PLAN_JSON_PARSE_ERROR",
          "JSON 校验失败",
          '{"cleaning_plan":["部分内容"}'
        ),
      ])
    ...
    expect(result.current.error).toEqual({
      ...
      message: "JSON 校验失败",
      partial_text: '{"cleaning_plan":["部分内容"}',
    });
...
describe("useStreamGenerateAnalysisPlan - 网络异常", () => {
...
describe("useStreamGenerateAnalysisPlan - cancel/reset", () => {
  it("cancel() 通过 AbortSignal 中断流式", async () => {
    const { Wrapper } = createWrapper();
    // 捕获传递给 streamGenerateAnalysisPlan 的 signal
    let capturedSignal: AbortSignal | undefined;
    vi.spyOn(api, "streamGenerateAnalysisPlan").mockImplementation(
      (pid: string, did: string, signal?: AbortSignal) => {
  ...
  it("初始状态正确", () => {
    const { Wrapper } = createWrapper();
    vi.spyOn(api, "streamGenerateAnalysisPlan").mockReturnValue(makeMockStream([]));
