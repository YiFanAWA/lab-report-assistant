/**
 * streamGenerateOutline API 单元测试（SPEC 0019 大纲生成流式化）。
 *
 * 测试覆盖：
...
describe("streamGenerateOutline - SPEC 0019", () => {
  ...
  it("项目 ID 被 URL 编码", async () => {
    ...
    await collectAll(streamGenerateOutline("proj with space"));
  ...
  it("传递 AbortSignal", async () => {
    ...
    await collectAll(streamGenerateOutline(PROJECT_ID, controller.signal));
  ...
  it("HTTP 错误透传后端结构化错误", async () => {
    ...
    await expect(
      collectAll(streamGenerateOutline("proj_missing"))
